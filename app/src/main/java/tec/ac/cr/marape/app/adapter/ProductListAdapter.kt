...
class ProductListAdapter(var productList: List<Product>, val inventory: Inventory) :
  ...
  override fun onBindViewHolder(holder: ViewHolder, position: Int) {
    ...
    holder.productDelete.setOnClickListener {
      message = "Este producto no se puede eliminar ya que se encuentra asociado. ¿Desea desligar y eliminar?"
  ...
  private fun sendConfirmation(context: Context, title: String, message: String, onYes: () -> Unit){
  ...
  private fun unlinkPerformance(currentProduct: Product, onComplete: (Boolean) -> Unit){
    db.collection("inventories").document(inventory.id)
      .update("items", FieldValue.arrayRemove(currentProduct.id))
  ...
  private fun showResultUnlink(context: Context, currentProduct: Product){
    unlinkPerformance(currentProduct){ success ->
      if (success) {
        Toast.makeText(context, "Producto desligado con éxito",
          Toast.LENGTH_SHORT
        ).show()
        notifyDataSetChanged()
      } else {
        Toast.makeText(context, "Error al desligar el producto",
          Toast.LENGTH_SHORT
        ).show()
      }
    }
  ...
  private fun showResultDelete(context: Context, productId: String){
    deletePerformance(productId){ success ->
      if (success) {
        Toast.makeText(context, "Producto eliminado con éxito",
          Toast.LENGTH_SHORT
        ).show()
        //notifyDataSetChanged()
      } else {
        Toast.makeText(context, "Error al eliminar el producto",
          Toast.LENGTH_SHORT
        ).show()
      }
    }
